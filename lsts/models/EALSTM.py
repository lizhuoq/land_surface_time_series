...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        input_size_dyn = configs.enc_in
        input_size_stat = configs.numeric_stat_in + 5 + 5 * 3 # numeric + climate + lc
        ...
        self.projection = nn.Linear(hidden_size, configs.c_out)

        self.climate_embed = nn.Embedding(26, 5)
        self.lc_embed = nn.Embedding(27, 5)
    ...
    def forward(self, x_d: torch.Tensor, numeric_s: torch.Tensor, climate_s: torch.Tensor, lc_s: torch.Tensor, mask=None) -> Tuple[torch.Tensor, torch.Tensor]:
        """[summary]
        ...
        x_s = torch.cat([numeric_s, self.climate_embed(climate_s)[:, 0, :], self.lc_embed(lc_s).reshape(batch_size, -1)], dim=1)
        ...
        output = self.predict_linear(h_n.transpose(1, 2)).transpose(1, 2)
